    ...
    <div className="w-full max-w-3xl mx-auto py-8 px-4">
      ...
      >
        <div>
          <button 
            onClick={onBack}
            className="text-sm font-medium text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white mb-2 transition-colors flex items-center gap-1"
          >
            ← Change destination
          </button>
          ...
          return (
            <motion.div
              ...
              className={`rounded-3xl border transition-all duration-500 overflow-hidden ${
                isActive 
                  ? 'bg-white dark:bg-slate-900 border-amber-200 dark:border-amber-900/50 shadow-xl ring-1 ring-amber-500/20' 
            ...
            >
              ...
              <div 
                ...
                className={`p-6 flex items-center justify-between cursor-pointer transition-colors ${
                  isActive ? 'bg-amber-50/50 dark:bg-amber-900/10' : ''
                }`}
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${
                    isActive ? 'bg-amber-100 dark:bg-amber-900/50 text-amber-600 dark:text-amber-400' : 
                    section.isComplete ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' :
                    'bg-slate-100 dark:bg-slate-800 text-slate-400'
                  }`}>
                    {section.isComplete && !isActive ? <Check className="w-6 h-6" /> : <Icon className="w-6 h-6" />}
                  </div>
                  <h3 className={`text-xl font-bold ${isActive ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'}`}>
                    {section.title}
                  </h3>
                </div>
                
                {!isActive && section.isComplete && (
                  <div className="text-sm font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-3 py-1 rounded-full">
                    Completed
                  </div>
                )}
              </div>
                    ...
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                      ...
                      {section.id === 'dates' && (
                        <div className="space-y-6">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                              <label className="block text-sm font-semibold text-slate-600 dark:text-slate-400 mb-2">Start Date</label>
                              <input
                                type="date"
                                value={store.startDate}
                                min={today}
                                onChange={(e) => store.setDates(e.target.value, store.endDate)}
                                className="w-full px-4 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-slate-900 dark:text-white font-medium focus:ring-2 focus:ring-amber-500/50 outline-none transition-all"
                              />
                            </div>
                            <div>
                              <label className="block text-sm font-semibold text-slate-600 dark:text-slate-400 mb-2">End Date</label>
                              <input
                                type="date"
                                value={store.endDate}
                      ...
                      {section.id === 'travelers' && (
                        <div className="space-y-6">
                              ...
                              <button
                                ...
                                className={`p-4 rounded-2xl border-2 flex flex-col items-center justify-center gap-2 transition-all ${
                                  store.travelerType === type.id
                                    ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 scale-105'
                                    : 'border-transparent bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300'
                                }`}
                              >
                          ...
                          {store.travelerType && (
                            <motion.div initial={{opacity:0}} animate={{opacity:1}} className="flex items-center justify-between bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl">
                              <span className="font-semibold text-slate-700 dark:text-slate-300">Traveler count:</span>
                              <div className="flex items-center gap-4">
                                <button onClick={() => store.setTravelerCount(Math.max(1, store.travelerCount - 1))} className="w-10 h-10 rounded-xl bg-white dark:bg-slate-700 shadow-sm flex items-center justify-center font-bold text-xl hover:bg-slate-100 dark:hover:bg-slate-600">-</button>
                                <span className="text-2xl font-bold w-8 text-center">{store.travelerCount}</span>
                                <button onClick={() => store.setTravelerCount(Math.min(20, store.travelerCount + 1))} className="w-10 h-10 rounded-xl bg-white dark:bg-slate-700 shadow-sm flex items-center justify-center font-bold text-xl hover:bg-slate-100 dark:hover:bg-slate-600">+</button>
                              </div>
                            </motion.div>
                          )}
                        </div>
        ...
        <button
          ...
          className={`flex items-center gap-3 px-8 py-4 rounded-full font-bold text-lg transition-all shadow-xl ${
            SECTIONS.every(s => s.isComplete)
              ? 'bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white hover:scale-105 hover:shadow-orange-500/25'
